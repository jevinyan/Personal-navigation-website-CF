  ...
  <div class="home-container">
    ...
    <footer class="footer">
      <div class="footer-content">
        ...
        <p class="copyright">Copyright © 2026 Nav-Item | <a href="https://github.com/jevinyan/vpn-tools" target="_blank" class="footer-link">Powered by Jevin</a></p>
      </div>
    </footer>
...
onMounted(async () => {
  ...
  for (const menu of menus.value) {
    const menuCards = await getCards(menu.id);
    allCardsData.push(...menuCards.data);
    for (const subMenu of menu.subMenus) {
      const subMenuCards = await getCards(menu.id, subMenu.id);
      allCardsData.push(...subMenuCards.data);
    }
  }
...
async function handleSearch() {
  ...
  if (selectedEngine.value.name === 'site') {
    ...
    if (match) {
      // 找到匹配的卡片，切换到对应的菜单
      const menu = menus.value.find(m => m.id === match.menu_id);
      if (menu) {
        activeMenu.value = menu;
        if (match.sub_menu_id) {
          activeSubMenu.value = menu.subMenus.find(sm => sm.id === match.sub_menu_id);
        } else {
          activeSubMenu.value = null;
        }
        await loadCards();
